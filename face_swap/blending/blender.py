...
from ..core.types import SwapResult, AlignedFace, Frame
...
class FaceBlender:
    ...
    def blend(
        ...
        if self.blend_mode == "poisson":
            ...
            result = self._feather_blend(frame, warped_face, warped_mask, feather_amount)
        else:  # alpha
    ...
    def _feather_blend(
        ...
        mask_feathered = cv2.GaussianBlur(mask_dilated, (feather_amount * 2 + 1, feather_amount * 2 + 1), 0)
    ...
    def _color_correct(
        ...
        for i in range(3):  # L, A, B channels
            ...
            if swapped_std > 0:
                # Match statistics
                result[:, :, i] = (swapped_chan - swapped_mean) * (target_std / swapped_std) + target_mean
