...
class SimSwapModel(FaceSwapper):
    ...
    def _load_onnx_model(self, path: str) -> None:
        ...
        self._output_names = [output.name for output in self._onnx_session.get_outputs()]
    ...
    def _run_onnx(self, target_img: np.ndarray, id_vector: np.ndarray) -> np.ndarray:
        ...
        inputs = {
            self._input_names[0]: target_img,
            self._input_names[1]: id_vector
        }
...
class IDInjectionGenerator(nn.Module):
    ...
    def forward(self, target_img: torch.Tensor, id_embedding: torch.Tensor) -> torch.Tensor:
